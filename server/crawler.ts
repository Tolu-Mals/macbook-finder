...
const getSellerInfo = (sellerInfoString: string) => {
  const regex = /Seller Information(\D+)(\d{1,3}%)Seller Score(\d+) FollowersFollowSeller PerformanceOrder Fulfillment Rate: ([A-Z]{1}[a-z]+)Quality Score: ([A-Z]{1}[a-z]+)Customer Rating: ([A-Z]{1}[a-z]+)/;
  const sellerInfoArr = sellerInfoString.match(regex);
  const name = sellerInfoArr?.at(1);
  const sellerScore = sellerInfoArr?.at(2);
  const followers = sellerInfoArr?.at(3);
  const orderFulfillmentRate = sellerInfoArr?.at(4);
  const qualityScore = sellerInfoArr?.at(5);
  const customerRating = sellerInfoArr?.at(6);
  return { name, followers, sellerScore, qualityScore, customerRating, orderFulfillmentRate };
}
        ...
        const detailsCrawler = new Crawler({
          ...
          callback: (error: Error, res: CrawlerRequestResponse, done: () => void) => {
            if(error) console.log(error);
              ...
              // console.log({ name, sellerScore, followers: Number(followers), orderFulfillmentRate, qualityScore, customerRating });
